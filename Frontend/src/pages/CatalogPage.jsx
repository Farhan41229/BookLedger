    ...
    <div className="pt-24 pb-20 min-h-screen bg-background relative overflow-hidden">
      ...
      {viewMode === 'discovery' && (
        <>
          ...
          <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-emerald-500/20 rounded-full blur-[100px] -z-10 mix-blend-screen animate-pulse" style={{ animationDelay: '2s' }} />
        </>
      )}
                ...
                >
                  <div className="space-y-4">
                    ...
                    <h1 className="text-4xl md:text-6xl font-bold tracking-tight text-foreground">
                      What kind of <span className="gradient-text">adventure</span> do you seek?
                    </h1>
                    <p className="text-muted-foreground text-lg md:text-xl">
                      Tell us what you're in the mood for, and we'll deal you 10 mystery books that match your vibe.
                    </p>
                  </div>

                  <form onSubmit={handleDiscover} className="w-full relative shadow-2xl rounded-2xl group">
                    ...
                    <div className="relative flex items-center bg-card border border-border/50 rounded-2xl p-2 focus-within:ring-2 focus-within:ring-primary/50 transition-all">
                      ...
                      >
                        {isSearching ? <Loader2 className="h-5 w-5 animate-spin" /> : 'Deal My Cards'}
                      </Button>
                    </div>
                  </form>
                </motion.div>
                ...
                >
                  <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left mb-8 bg-card/40 p-6 rounded-2xl border border-border/50 shadow-sm backdrop-blur-sm">
                    <div>
                      ...
                      <p className="text-muted-foreground mt-2">Pick a card to reveal a book matching: <span className="font-medium text-foreground italic">"{prompt}"</span></p>
                    </div>
                    <Button variant="outline" onClick={resetDiscovery} className="shrink-0 bg-background/50 hover:bg-background shadow-none">
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="absolute inset-0 backface-hidden rotate-y-0-front bg-card border-2 border-primary/20 rounded-xl shadow-xl flex flex-col items-center justify-center p-6 text-center hover:border-primary/50 hover:shadow-primary/20 transition-all">
                              ...
                              <h3 className="text-2xl font-serif font-bold text-foreground/80">Mystery Book</h3>
                              <p className="text-muted-foreground mt-2 font-medium">Click to reveal</p>
                            </div>
                               ...
                               <div className="p-4 flex-1 flex flex-col justify-between">
                                 <div className="space-y-2">
                                   <div className="flex justify-between items-center">
                                     <span className="font-bold text-lg text-primary">${Number(book.price).toFixed(2)}</span>
                                 ...
                                 <div className="flex gap-2 mt-4 pt-4 border-t border-border/50">
                                   <Button 
                                     ...
                                     size="sm" 
                                     className="flex-1"
                                     onClick={(e) => { e.stopPropagation(); handleSkip(index); }}
                                   >
                                     Skip
                                   </Button>
                                   <Button 
                                     ...
                                     onClick={(e) => { e.stopPropagation(); handleAddToCart(book, index); }}
                                   >
            ...
            >
              <div className="mb-10 text-center max-w-2xl mx-auto space-y-4">
                <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-foreground">
                  Explore Our <span className="text-foreground">Collection</span>
                </h1>
                <p className="text-muted-foreground text-lg">
                  Browse gracefully through thousands of books across all genres.
                </p>
              </div>
              ...
              {loadingBooks ? (
                ...
                <div className="text-center h-64 flex flex-col items-center justify-center space-y-3 bg-muted/20 rounded-2xl border border-dashed">
                  ...
                  <p className="text-xl text-muted-foreground font-medium">No books found.</p>
                  <Button variant="outline" onClick={() => { setSearch(''); setSelectedGenre('All'); }}>Clear Filters</Button>
                </div>
              ) : (
                        ...
                        <Card className="h-full overflow-hidden border-transparent hover:border-border/50 bg-card/40 hover:bg-card hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 group">
                          ...
                          <CardContent className="p-4 space-y-2">
                            ...
                            <div className="pt-2 flex items-center justify-between">
                              <span className="font-bold text-lg">${Number(book.price).toFixed(2)}</span>
                              {book.genre && (
                                <Badge variant="secondary" className="text-[10px] px-2 font-normal bg-secondary/50">
                                  {book.genre}
                                </Badge>
                              )}
                            </div>
